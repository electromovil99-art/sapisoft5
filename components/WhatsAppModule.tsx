...
import { Send, MessageCircle, User, Loader2, Users, FileText, Paperclip, Settings, PlusCircle, ArrowLeft, Trash2, Edit2, Save, Filter, Briefcase, Image as ImageIcon, Search, X, Video, File } from 'lucide-react';
import { io, Socket } from 'socket.io-client';
import { QRCodeSVG } from 'qrcode.react';

// URL NGROK (¡Actualízala si cambia!)
...
export default function WhatsAppModule({ clients = [] }: { clients: any[] }) {
  ...
  const fetchFullDb = async () => { const db = await api('/crm/all'); setFullCrmDb(db); setStages(db.stages || []); };
  ...
  const handleNewMsg = (msg: any) => {
      setSelectedChatId(prev => {
          if(prev === (msg.fromMe ? msg.to : msg.from)) { setMessages(m => [...m, msg]); setTimeout(()=>msgsEndRef.current?.scrollIntoView({ behavior: "smooth" }), 100); }
          return prev;
      });
      fetchChats();
  };
  ...
  const sendBulk = async () => {
      ...
      if(confirm(`¿Enviar a ${finalNumbers.length} contactos?`)) {
          ...
          try { await api('/bulk/send', { method: 'POST', body: JSON.stringify({ numbers: finalNumbers, message: bulkMessage, media: bulkFile, minDelay, maxDelay, sendTogether }) }); } 
  ...
  // RENDERIZADOR DE MENSAJES INTELIGENTE
  const renderMessageContent = (m: any) => {
      const isMedia = m.type === 'image' || m.type === 'video' || m.type === 'document' || m.hasMedia;
      
      return (
          <div className={`max-w-[85%] md:max-w-[60%] p-2 rounded-lg text-sm shadow relative break-words flex flex-col gap-1 ${m.fromMe?'bg-[#d9fdd3] rounded-tr-none':'bg-white rounded-tl-none'}`}>
              
              {/* ZONA DE MULTIMEDIA */}
              {isMedia && (
                  <div className="bg-black/5 rounded p-2 flex items-center gap-2 mb-1 border border-black/5">
                      {m.type === 'video' ? <Video size={20} className="text-slate-500"/> : 
                       m.type === 'document' ? <File size={20} className="text-slate-500"/> : 
                       <ImageIcon size={20} className="text-slate-500"/>}
                      
                      <span className="text-xs italic text-slate-500 font-bold">
                          {m.type === 'image' ? '📷 Foto' : m.type === 'video' ? '🎥 Video' : '📄 Archivo'}
                      </span>
                  </div>
              )}
              ...
              {/* HORA */}
              <span className="text-[9px] text-slate-400 block text-right mt-1 select-none">
  ...
  const renameStage = async (oldName: string) => {
      ...
      setFullCrmDb((prev:any) => { const n = {...prev.clients}; Object.keys(n).forEach(k => { if(n[k].stage === oldName) n[k].stage = tempStageName; }); return { ...prev, clients: n }; });
  ...
  return (
    <div className="flex h-full w-full bg-white overflow-hidden text-slate-800 font-sans relative">
      ...
      <div className={`flex flex-col bg-slate-50 border-r h-full ${showMobileChat ? 'hidden md:flex md:w-80' : 'w-full md:w-80'}`}>
          ...
          <div className="flex text-[10px] font-bold border-b bg-white shrink-0">
              {['CHAT','BULK','CONFIG'].map(t => (<button key={t} onClick={()=>setActiveTab(t as any)} className={`flex-1 p-3 ${activeTab===t?'text-emerald-600 border-b-2 border-emerald-600':''}`}>{t}</button>))}
          </div>
          <div className="p-2 bg-white border-b shrink-0">
              <div className="flex items-center gap-2 bg-slate-100 rounded-lg px-2 py-1.5 border"><Search size={14} className="text-slate-400"/><input className="bg-transparent outline-none text-xs w-full" placeholder="Buscar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}/>{searchTerm && <button onClick={()=>setSearchTerm("")} className="text-slate-400"><X size={12}/></button>}</div>
          </div>
          <div className="flex-1 overflow-y-auto min-h-0">
              {activeTab === 'CHAT' && (
                  <>
                    <div className="p-2 bg-slate-50 border-b sticky top-0 z-10">
                        <div className="flex items-center gap-2 bg-white rounded border p-1"><Filter size={14} className="text-slate-400"/><select className="w-full text-xs outline-none bg-transparent" value={chatFilterStage} onChange={e=>setChatFilterStage(e.target.value)}><option value="Todos">Todas las Etapas</option>{stages.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    </div>
                    {filteredChatList.map(c => (
                        <div key={c.id._serialized} onClick={() => loadChat(c.id._serialized)} className={`p-3 border-b cursor-pointer hover:bg-slate-100 ${selectedChatId===c.id._serialized?'bg-emerald-50':''}`}>
                            ...
                            <div className="flex justify-between items-center mt-1"><span className="text-xs text-slate-500 truncate w-32 flex items-center gap-1">{c.lastMessage?.hasMedia && <ImageIcon size={10}/>} {c.lastMessage?.body || (c.lastMessage?.hasMedia ? 'Multimedia' : '')}</span>{fullCrmDb.clients[c.id.user]?.stage && <span className="text-[9px] px-1 bg-blue-100 text-blue-700 rounded border border-blue-200">{fullCrmDb.clients[c.id.user].stage}</span>}</div>
                        </div>
                    ))}
                  </>
              )}
              {activeTab === 'BULK' && (
                  <div className="p-2 space-y-3">
                      <div className="bg-white p-2 rounded border"><label className="text-xs font-bold block mb-1">Filtrar por Etapa</label><select className="w-full border text-xs p-1 rounded" value={bulkFilterStage} onChange={e=>setBulkFilterStage(e.target.value)}><option value="Todas">Todas</option>{stages.map(s=><option key={s} value={s}>{s}</option>)}</select><div className="text-[10px] mt-1 text-blue-600 font-bold">Resultados: {filteredBulkList.length}</div></div>
                      <div className="max-h-60 overflow-y-auto border rounded bg-white">
                          <div className="p-1 bg-slate-50 border-b flex justify-between items-center"><span className="text-[10px] font-bold text-slate-500">CLIENTES</span><button onClick={()=>setSelectedBulkClients(filteredBulkList.map(c=>c.phone))} className="text-[9px] text-blue-600 hover:underline">Seleccionar Todos</button></div>
                          {filteredBulkList.map((c:any, i:number) => (<div key={i} className="flex gap-2 items-center p-2 border-b hover:bg-slate-50"><input type="checkbox" checked={selectedBulkClients.includes(c.phone)} onChange={()=>{setSelectedBulkClients(prev=>prev.includes(c.phone)?prev.filter(p=>p!==c.phone):[...prev, c.phone])}}/><div className="text-xs"><div className="font-bold">{c.name}</div><span className="bg-slate-100 px-1 rounded text-[9px]">{c.stage}</span></div></div>))}
                      </div>
                      <div className="bg-white p-2 rounded border"><label className="text-xs font-bold block mb-1">Números Manuales</label><textarea className="w-full border text-xs p-1 rounded font-mono" rows={3} placeholder="999888777..." value={manualNumbers} onChange={e=>setManualNumbers(e.target.value)}/></div>
                  </div>
              )}
              {activeTab === 'CONFIG' && (
                  <div className="p-4 space-y-4">
                      <div>
                          ...
                          <div className="space-y-1">{filteredStages.map((s, i) => (<div key={i} className="flex justify-between items-center bg-white p-2 rounded border text-sm">{editingStageIndex === i ? (<div className="flex gap-1 w-full"><input className="flex-1 border p-1 text-xs" autoFocus defaultValue={s} onChange={(e)=>setTempStageName(e.target.value)}/><button onClick={()=>renameStage(s)} className="text-green-600"><Save size={14}/></button></div>) : (<><span>{s}</span><div className="flex gap-2"><button onClick={()=>{setEditingStageIndex(i); setTempStageName(s);}} className="text-blue-500"><Edit2 size={14}/></button></div></>)}</div>))}</div>
                      </div>
                  </div>
              )}
          </div>
          ...
          {activeTab === 'BULK' ? (
             ...
             <div className="flex-1 flex flex-col h-full min-h-0">
                <div className="p-2 bg-slate-100 border-b flex justify-between items-center shadow-sm shrink-0">
                    <div className="flex items-center gap-2"><button onClick={()=>setShowMobileChat(false)} className="md:hidden p-1 bg-white rounded-full"><ArrowLeft size={16}/></button><span className="font-bold text-slate-800 truncate max-w-[150px]">{chats.find(c=>c.id._serialized===selectedChatId)?.name}</span></div>
                    {crmClient && (<select className="text-xs border rounded p-1 bg-white text-emerald-700 font-bold outline-none" value={crmClient.stage} onChange={(e) => updateCrm('stage', e.target.value)}>{stages.map(s => <option key={s} value={s}>{s}</option>)}</select>)}
                </div>
                {/* LISTA DE MENSAJES (Ahora renderiza Media) */}
